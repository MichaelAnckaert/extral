...
class JSONConnector(FileConnector):
    ...
    def __init__(self, config: FileConfig):
        ...
        Args:
            config: FileConfig with JSON-specific options
        """
    ...
    def extract_data(
        ...
        logger.info(f"Extracting data from JSON file: {path} (format: {self.json_format})")
    ...
    def load_data(
        ...
        if strategy != LoadStrategy.REPLACE:
            raise ValueError(f"JSON connector only supports REPLACE strategy, got {strategy}")
        
        logger.info(f"Loading {len(data)} records to JSON file: {path} (format: {self.json_format})")
